...
OscClientRow::OscClientRow(const OscClient& client, QWidget* parent) : QFrame(parent) {
    ...
    targetSourceCombo->setStyleSheet("background: rgba(0,0,0,0.3); border: 1px solid rgba(255,255,255,0.1); color: white;");
...
void OscSettingsDialog::setupUi() {
    ...
    emitDetailsBtn->setStyleSheet("background-color: #fab387; color: #11111b; font-weight: bold; padding: 6px; border-radius: 4px;");
    ...
    startStopBtn->setStyleSheet("QPushButton { background-color: #a6e3a1; color: #11111b; font-weight: bold; padding: 6px; border-radius: 4px; }");
    ...
    mongooseGb->setStyleSheet("QGroupBox { font-weight: bold; border: 1px solid rgba(255,255,255,0.1); margin-top: 12px; padding-top: 12px; color: #f9e2af; } QGroupBox::title { subcontrol-origin: margin; left: 8px; padding: 0 4px; }");
    ...
    mongoosePortSpin->setStyleSheet("background: #313244; border: 1px solid #45475a; color: white; padding: 4px; border-radius: 4px;");
    ...
    startStopMongooseBtn->setStyleSheet("QPushButton { background-color: #a6e3a1; color: #11111b; font-weight: bold; padding: 6px; border-radius: 4px; }");
    ...
    routeGb->setStyleSheet("QGroupBox { font-weight: bold; border: 1px solid rgba(255,255,255,0.1); color: #94e2d5; }");
    ...
    clientsGb->setStyleSheet("QGroupBox { font-weight: bold; border: 1px solid rgba(255,255,255,0.1); color: #89b4fa; }");
    ...
    toggleLogBtn->setStyleSheet("QPushButton { background: rgba(255,255,255,0.05); color: #89b4fa; border: none; padding: 4px; border-radius: 4px; text-align: left; } QPushButton:hover { background: rgba(255,255,255,0.1); }");
    ...
    connect(emitDetailsBtn, &QPushButton::clicked, []() {
        EmitServerDetails();
    });
...
void OscSettingsDialog::onRefreshStatus() {
    ...
    startStopBtn->setStyleSheet(running ? "background-color: #ff453a; color: white;" : "background-color: #a6e3a1; color: #11111b;");

    // Mongoose Status
    bool mRunning = mgr.IsMongooseRunning();
    mongooseStatusDot->setStyleSheet(mRunning ? "background-color: #39d353; border-radius: 5px;" : "background-color: #ff6b6b; border-radius: 5px;");
    mongooseStatusLabel->setText(mRunning ? "Running" : "Stopped");
    startStopMongooseBtn->setText(mRunning ? "Stop" : "Start");
    startStopMongooseBtn->setStyleSheet(mRunning ? "background-color: #ff453a; color: white;" : "background-color: #a6e3a1; color: #11111b;");
    
    // Live port availability check for spins if not running
    if (!running) {
        bool avail = OscManager::IsPortAvailable(serverPortSpin->value());
        serverPortSpin->setStyleSheet(avail ? "background: #313244; color: #a6e3a1;" : "background: #313244; color: #ff453a;");
    } else {
    ...
    if (!mRunning) {
        ...
        mongoosePortSpin->setStyleSheet(avail ? "background: #313244; color: #a6e3a1;" : "background: #313244; color: #ff453a;");
    } else {
...
void OscSettingsDialog::onAddClient() {
    ...
    QStringList sources;
    for (int i = 0; i < targetSourceCombo->count(); i++) sources << targetSourceCombo->itemText(i);
...
void OscSettingsDialog::loadSettings() {
    ...
    if (targetIdx != -1) targetSourceCombo->setCurrentIndex(targetIdx);
    ...
    for (auto* row : clientRows) row->deleteLater();
    clientRows.clear();

    QStringList sources;
    for (int j = 0; j < targetSourceCombo->count(); j++) sources << targetSourceCombo->itemText(j);
